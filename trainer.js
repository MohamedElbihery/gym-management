...
const TrainerModule = {
    ...
    renderDashboard() {
        ...
        s.innerHTML = `
            <div class="section-header"><h1>${I18n.t('trainer.dashTitle')}</h1><p class="section-subtitle">${I18n.t('trainer.dashSubtitle')}</p></div>
            <div class="kpi-grid">
                ...
                <div class="kpi-card"><div class="kpi-icon green"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M22 11.08V12a10 10 0 1 1-5.93-9.14"/><polyline points="22 4 12 14.01 9 11.01"/></svg></div><div class="kpi-data"><span class="kpi-value">${todayCheckins.length}</span><span class="kpi-label">${I18n.t('dash.checkedInToday')}</span></div></div>
                <div class="kpi-card"><div class="kpi-icon orange"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><polyline points="22 12 18 12 15 21 9 3 6 12 2 12"/></svg></div><div class="kpi-data"><span class="kpi-value">${thisWeek.length}</span><span class="kpi-label">${I18n.t('trainer.weeklyCheckins')}</span></div></div>
                <div class="kpi-card"><div class="kpi-icon purple"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M21 15a2 2 0 0 1-2 2H7l-4 4V5a2 2 0 0 1 2-2h14a2 2 0 0 1 2 2z"/></svg></div><div class="kpi-data"><span class="kpi-value">${Store.getA('feedbacks').length}</span><span class="kpi-label">${I18n.t('trainer.feedbacksSent')}</span></div></div>
            </div>
            <div class="card"><h3 class="card-title">${I18n.t('trainer.recentCheckins')}</h3>
                ${todayCheckins.length === 0 ? `<p class="empty-state sm">${I18n.t('trainer.noCheckinsToday')}</p>` : `<div class="meal-list">${todayCheckins.slice(0, 10).map(a => `<div class="meal-item"><span class="meal-name">${a.userName}</span><span style="color:var(--text-muted);font-size:0.82rem;">${formatTime(a.time)}</span></div>`).join('')}</div>`}
            </div>
            <div class="card"><h3 class="card-title">${I18n.t('trainer.atRisk')}</h3>
                <div>${this.getRiskMembers().map(m => `<div class="risk-card"><div class="risk-avatar" style="background:var(--red)">${m.name.charAt(0)}</div><div class="risk-info"><div class="risk-name">${m.name}</div><div class="risk-reason">${m.riskReason}</div></div><button class="btn btn-ghost btn-sm" onclick="TrainerModule.openFeedback('${m.id}','${m.name}')">${I18n.t('trainer.sendMessage')}</button></div>`).join('') || `<p class="empty-state sm">${I18n.t('trainer.noRisk')}</p>`}</div>
            </div>
        `;
    },
    ...
    getRiskMembers() {
        ...
        members.forEach(m => {
            ...
            const reasons = [];
            if (recentCheckins === 0) reasons.push(I18n.t('trainer.riskNoCheckins'));
            if (m.streak === 0) reasons.push(I18n.t('trainer.riskLostStreak'));
            if (m.subscription?.status === 'expiring') reasons.push(I18n.t('trainer.riskExpiring'));
    ...
    renderMembers() {
        ...
        s.innerHTML = `
            <div class="section-header"><h1>${I18n.t('trainer.membersTitle')}</h1><p class="section-subtitle">${I18n.t('trainer.membersSubtitle')}</p></div>
            ${members.map((m, i) => {
            ...
            return `<div class="trainer-member-card">
                    ...
                    <div class="tm-info">
                        ...
                        <div class="tm-meta">${I18n.t('auth.goal')}: ${I18n.t('auth.' + (m.goal || ''))} · ${recentCount} ${I18n.t('trainer.workoutsWk')} · ${I18n.t('auth.weight').split(' ')[0]}: ${latest ? latest.weight + 'kg' : (m.weight || '—') + 'kg'} · ${I18n.t('dash.streakLabel').split(' ')[0]}: 🔥${m.streak || 0}</div>
                    </div>
                    <div class="tm-actions">
                        <button class="btn btn-ghost btn-sm" onclick="TrainerModule.openFeedback('${m.id}','${m.name}')">${I18n.t('nav.feedback')}</button>
                    </div>
                </div>`;
        }).join('')}
        `;
    ...
    renderPlans() {
        ...
        s.innerHTML = `
            <div class="section-header"><h1>${I18n.t('trainer.plansTitle')}</h1><p class="section-subtitle">${I18n.t('trainer.plansSubtitle')}</p></div>
            ${members.map(m => {
            ...
            return `<div class="card" style="margin-bottom:16px;">
                    <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:12px;">
                        <div><strong>${m.name}</strong> <span style="color:var(--text-muted);font-size:0.82rem;">· ${I18n.t('auth.' + (m.goal || ''))} · ${I18n.t('auth.' + (m.experience || ''))}</span></div>
                        <button class="btn btn-ghost btn-sm" onclick="TrainerModule.regenerateForMember('${m.id}')">🤖 ${I18n.t('plan.regenerate').split(' ')[1]}</button>
                    </div>
    ...
    regenerateForMember(userId) {
        ...
        Store.set('workout_plans', plans);
        showToast(I18n.t('trainer.planRegenFor', [user.name]));
        Notifs.push('Plan Updated', I18n.t('trainer.coachUpdatedPlan'));
        this.renderPlans();
    },
    ...
    renderFeedback() {
        ...
        s.innerHTML = `
            <div class="section-header"><h1>${I18n.t('trainer.feedbackTitle')}</h1><p class="section-subtitle">${I18n.t('trainer.feedbackSubtitle')}</p></div>
            ...
            <div class="card"><h3 class="card-title">${I18n.t('trainer.sentFeedback')}</h3>
                ${feedbacks.length === 0 ? `<p class="empty-state sm">${I18n.t('trainer.noFeedbackSentYet')}</p>` : feedbacks.map(f => `<div class="notif-item" style="margin-bottom:8px;"><div class="notif-title">${I18n.t('feedback.to')}: ${f.memberName}</div><div>${f.message}</div><div class="notif-time">${timeAgo(f.createdAt)}</div></div>`).join('')}
            </div>
        `;
    },
    ...
    sendFeedback(e) {
        ...
        showToast(I18n.t('trainer.feedbackSentTo', [memberName]));
